...
server.get(
    ...
    function (req, res, next) {
        ...
        if (
            req.querystring.registration &&
            req.querystring.registration === "submitted"
        ) {
        ...
        res.render("account/accountDashboard", {
            ...
            viewSavedPaymentsUrl: URLUtils.url(
                "PaymentInstruments-List"
            ).toString(),
            addPaymentUrl: URLUtils.url(
...
server.post(
    ...
    function (req, res, next) {
        ...
        var customerLoginResult = accountHelpers.loginCustomer(
            email,
            password,
            rememberMe
        );
        ...
        if (customerLoginResult.authenticatedCustomer) {
            res.setViewData({
                authenticatedCustomer:
                    customerLoginResult.authenticatedCustomer,
            });
            res.json({
                success: true,
...
server.post(
    ...
    function (req, res, next) {
        ...
        var receivedMsgHeading = Resource.msg(
            "label.resetpasswordreceived",
            "login",
            null
        );
        var receivedMsgBody = Resource.msg(
            "msg.requestedpasswordreset",
            "login",
            null
        );
        ...
        if (email) {
            ...
            if (isValid) {
                ...
                if (resettingCustomer) {
                    accountHelpers.sendPasswordResetEmail(
                        email,
                        resettingCustomer
                    );
                }
...
server.get(
    ...
    function (req, res, next) {
        ...
        if (!resettingCustomer) {
            ...
            res.render("account/password/newPasswordRedirect", {
                token: token,
            });
        }
        next();
...
server.get("Header", server.middleware.include, function (req, res, next) {
    var template = req.querystring.mobile
        ...
        : "account/header";
    res.render(template, {
        name: req.currentCustomer.profile
            ? req.currentCustomer.profile.firstName
            : null,
    });
    next();
});
